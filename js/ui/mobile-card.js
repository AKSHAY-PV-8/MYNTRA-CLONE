...
const mobileCardContainer = document.getElementById("mobile-card__section");
let mobileCardList = ``;


const mobileCards = async() =>{



    const data = await fetchProducts();
    const prodoctData = [...data];


    for(let productDetails of prodoctData){
        ...
        <li class="mobile-card">
                        <div class="mobile-card__base">
                            <picture>
                                <source srcset="${source}" type="image/webp">
                                <img src="${src}" class="img-responsive preLoad loaded" alt="" title="" style="width: 100%;">
                            </picture>

                            <div class="mobile-product-rating__container">
                                <span class="mobile-rating__text">${rating}</span>
                                <span class="mobile-rating__star"></span>
                                <span style="color: rgb(148, 150, 159);">|</span>
                                <span class="mobile-rating__count">${ratingCount}</span>
                            </div>
                            
                            ${waterMark === true ? `<div class="ad__container">AD</div>` : ``}
                            

                            <div class="mobile-card__details-section">
                                <div class="mobile-product__details-wrap">
                                    <h3>${brand}</h3>
                                    <h4>${name}</h4>
                                    <div class="mobile-product-price__container">
                                        <div class="mobile-product-price">
                                            ...
                                            <span class="mobile__price-value">${discountPrice}</span>
                                            <div class="mobile-cross-amount__section">
                                                ...
                                                <span class="mobile-cross__amount">${price}</span>
                                            </div>
                                            <div class="mobile-discount__container">
                                                <span>(${discountPercentage} OFF)</span>
                                                
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card-padded__section"></div>
                                </div>

                                <div class="mobile-wish-list__icon">
...
export const renderMobileCard = async() => {
    await mobileCards();

    mobileCardContainer.innerHTML = `${mobileCardList}`
